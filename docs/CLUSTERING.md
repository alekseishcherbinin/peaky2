...
6. **Flat-family demotion** (`split_flat_clusters`). For each surviving family
   ...
   **max ÷ median of the member-mean** trace. If **< `FLAT_CLUSTER_RANGE` (1.4)**
   ...
   a whole doesn't move *much*"). Otherwise it is a **dynamic co-varying family**.
   ⚠ This is a pure **magnitude/amplitude** test — it measures *how much* the family
   moves, **not when** and **not whether the movement has coherent structure** (see §8).
