...
def run_ladder_gapfill(client, sample_id: str, ledger: pd.DataFrame, profile,
    ...
    if not len(anchors):
        return out
    existing = set(m0["neutral_formula"].dropna())
    ...
    for _, a in anchors.iterrows():
        ...
        for uname, delta in _UNITS.items():
            for k in range(1, max_steps + 1):
                ...
                if pid is None:
                    continue
                # G3/G4: the peak must be free (or weak) and not an iso satellite
                i0 = ledger.index[ledger["peak_id"] == pid][0]
                role = str(ledger.at[i0, "role"]); h = float(ledger.at[i0, "height"])
                if role == L.ROLE_M0:
                    score = ledger.at[i0, "ion_score"]
                    if pd.notna(score) and float(score) >= 0.90:
    ...
    for pid, p in proposals.items():
        ...
        # G6: confidence capped by anchor strength; commit only Good/Low (never High)
        ...
        if conf.startswith("Reject"):
            continue
        try:
            if L.role_of(ledger, pid) == L.ROLE_M0:
                # G5: weak existing M0 -> overwrite; (strong already filtered)
                if not bool(ledger.at[ledger.index[ledger["peak_id"] == pid][0], "locked"]):
                    pass
                else:
                    continue
            ...
            L.commit_assignment(
                ...
                overwrite=(L.role_of(ledger, pid) == L.ROLE_M0))
            out["committed"] += 1
